...
# change kappa and erepro based on shiny exploration
  ...
  params <- setParams(params,kappa=10^6.3)
...
getError <- function(vary=vals,params=params,dat=preddat,data_type="biomass",timetorun=100,whichspecies=ind) {
  ...
  sim <- project(params, effort = 0, t_max = timetorun, dt=0.1)
          ...
          output <-getBiomass(sim)[timetorun,]   # units here: g. m2
          
          simbio<-data.frame(species=mf.params@species_params$species,w_inf=mf.params@species_params$w_inf,simbio=           output/1e6) # convert units to tperm2
...
params <- setParams(params,kappa=1e5)
# optim values:
...
## check obs vs pred
pred_bio <-melt(getBiomass(sim)[100,]/1e6)
pred_bio$species <- row.names(pred_bio)
pred_bio<-as.data.frame(merge(pred_bio,preddat,by.x="species",all=T))
...
# What would happen if changed parameters above based on different assumptions? Need to chekc all of these make sense!
