...
params <- setParams(params,kappa=10^7.1)
...
```{r}
#sim<-newsim if using results from below
## check obs vs pred
...
optim_result <-optimParallel(par=vary,getError,params=shinyparams,method   ="L-BFGS-B",lower=c(rep(1e-4,12)),upper= c(rep(6,12)),
                            parallel=list(loginfo=TRUE, forward=TRUE))

stopCluster(cl)

saveRDS(optim_result,"optim_result.RDS")

# optim values:
params@species_params$R_max <- 10^optim_result$par 


# set the param object - oh no optimised using kappa 10^7.1
newparams<-setParams(params,kappa=10^7.1)
...
params@species_params$R_max <- rep(1e-4,12) 
newparams<-setParams(params,kappa=10^5.6)
newsim <- project(params, effort = 0, t_max = 500, dt=0.1,initial_n = sim@n[100,,],initial_n_pp = sim@n_pp[100,])
plot(newsim)
